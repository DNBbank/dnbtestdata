...
categories = pd.DataFrame([['Loans&Rent',1,1,'Out',10000,0],
                    ...
                    ['Insurance',16,1,'Out',150,0]], columns=['CategoryName','CategoryLabel','RegularInd','InOut','Mean','MaxTrans'])
...
for i in range(nr_cust):
    ...
    count = 0
    while ((count < nr_trx_i) or SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)<min_acct_sum_i or SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)>max_acct_sum_i):
        cat = np.random.choice(np.arange(1,nr_cat+1), p = cat_prob)
        done = np.zeros(16)
        while done[cat-1] == 1 or (SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)<min_acct_sum_i and categories.loc[categories['CategoryLabel'] == cat,'InOut'].iloc[0] == 'Out') or (SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)>max_acct_sum_i and categories.loc[categories['CategoryLabel'] == cat,'InOut'].iloc[0] == 'In'):
            cat = np.random.choice(np.arange(1,nr_cat+1), p = cat_prob)
        if categories.loc[categories['CategoryLabel'] == cat,'RegularInd'].iloc[0]>0: 
            ...
            while date_ < end:
                tmp_df = pd.concat([tmp_df, pd.DataFrame([[i,date_,categories.loc[categories['CategoryLabel'] == cat, 'CategoryName'].iloc[0],amt]],columns=['AccountID','Date','Category','Amount'])])
            ...
            for j, date_ in enumerate(dates):
                tmp_df = pd.concat([tmp_df, pd.DataFrame([[i,date_,categories.loc[categories['CategoryLabel'] == cat, 'CategoryName'].iloc[0],amt[j]]],columns=['AccountID','Date','Category','Amount'])])
                count = count + 1
            done[cat-1] = 1
        else:
            tmp_df = pd.DataFrame(columns=['AccountID','Date','Category','Amount'])
...
for i in range(nr_cust):
    ...
    categories_ = categories[categories['AgeGroup'] == seg]
    nr_trx_i = min_nr_trx[i]
    max_acct_sum_i = max_acct_sum[i]
    min_acct_sum_i = min_acct_sum[i]
    count = 0
    while ((count < nr_trx_i) or SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)<min_acct_sum_i or SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)>max_acct_sum_i):
        ...
        while done[cat-1] == 1 or (SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)<min_acct_sum_i and categories_.loc[categories_['CategoryLabel'] == cat,'InOut'].iloc[0] == 'Out') or (SynthData.loc[SynthData['AccountID'] == i,'Amount'].agg(sum)>max_acct_sum_i and categories_.loc[categories_['CategoryLabel'] == cat,'InOut'].iloc[0] == 'In'):
            cat = np.random.choice(np.arange(1,nr_cat+1), p = cat_prob)
        if categories_.loc[categories_['CategoryLabel'] == cat,'RegularInd'].iloc[0]>0: 
            ...
            date_ = date(2017,start_month,day)
            while date_ < end:
                tmp_df = pd.concat([tmp_df, pd.DataFrame([[i,date_,categories.loc[categories['CategoryLabel'] == cat, 'Category'].iloc[0],amt]],columns=['AccountID','Date','Category','Amount'])])
                date_ = custom_delta(date_, freq, date_.month, end)
            count = count + len(tmp_df.index)
            if cat == 12: #Salary
                done[cat-1] = 1
            ...
            dates = np.random.choice(year_range,nr_trx_cat,replace = True)
            for j, date_ in enumerate(dates):
                tmp_df = pd.concat([tmp_df, pd.DataFrame([[i,date_,categories.loc[categories['CategoryLabel'] == cat, 'Category'].iloc[0],amt[j]]],columns=['AccountID','Date','Category','Amount'])])
                count = count + 1
            done[cat-1] = 1
        else:
            tmp_df = pd.DataFrame(columns=['AccountID','Date','Category','Amount'])
...
'''
    100 cust: 413 sec on 45554 trx
    
    1 cust: 4.13 sec 
    10000 cust: 41300sec = 11.5hrs
    ~1mill trans: 9066sec = 2.5hrs
    ~3mill trans: 27198sec = 7.5hrs
    ~10mill trans: 90661sec = 25hrs
